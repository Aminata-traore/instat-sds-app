  ...
  useEffect(() => {
    (async () => {
      ...
      const [s1, r1, i1, sv1, p1, sf1] = await Promise.all([
        supabase.from("ref_structures").select("code, structures, abreviation").order("code", { ascending: true }),
      ...
      setStructures((s1.data ?? []) as any);
      setResultats((r1.data ?? []) as any);
      setIndicateurs((i1.data ?? []) as any);
      setSourcesVerif((sv1.data ?? []) as any);
      setProduits((p1.data ?? []) as any);
      setSourcesFinance((sf1.data ?? []) as any);
    })();
  }, []);
    ...
    return {
      ...
      data: {
        section1: {
          region: region.trim(),
          cercle: cercle.trim(),
          structure_realisation_code: structureCode,
          structure_collecte_code: collecteStructureCode,
        ...
        section2: {
          ...
          envergure: { code: envergure, autre: envergure === "7" ? envergureAutre.trim() : null },
  ...
  const save = async (mode: SaveMode) => {
    ...
    try {
      ...
      if (mode === "soumis") {
        // reset soft
        setRegion(""); setCercle("");
        setStructureCode(""); setCollecteStructureCode("");
        setIntituleActivite(""); setResultatCode("");
        setResponsable(""); setAnnee(""); setNumeroFiche("");
        setDonneesDesag("2");
        setIndicateursCodes([]);
        setEnvergure(""); setEnvergureAutre("");
        setProgrammee("1");
        setSourcesVerifCodes([]); setSourceVerifAutre("");
        setProduitsCodes([]); setProduitAutre("");
        setFinancementCodes([]); setFinancementAutre("");
      }
    } catch (e: any) {
    ...
    <main style={{ padding: 24, maxWidth: 980 }}>
      ...
      {refError && (
        <p style={{ marginTop: 12, color: "crimson" }}>
          Erreur chargement référentiels: {refError}
        </p>
      )}
      ...
      <section style={{ marginTop: 18, padding: 14, border: "1px solid #e5e7eb", borderRadius: 10 }}>
        ...
        <div style={{ marginTop: 12, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <input placeholder="1.01 Région" value={region} onChange={(e) => setRegion(e.target.value)} />
          <input placeholder="1.02 Cercle" value={cercle} onChange={(e) => setCercle(e.target.value)} />
          ...
          <select value={structureCode} onChange={(e) => setStructureCode(e.target.value)}>
            ...
            {structures.map((s) => (
              <option key={s.code} value={s.code}>
                {s.code} — {s.structures}{s.abreviation ? ` (${s.abreviation})` : ""}
              </option>
            ))}
          </select>

          <select value={collecteStructureCode} onChange={(e) => setCollecteStructureCode(e.target.value)}>
          ...
          <select value={resultatCode} onChange={(e) => setResultatCode(e.target.value)}>
      ...
      <section style={{ marginTop: 18, padding: 14, border: "1px solid #e5e7eb", borderRadius: 10 }}>
        ...
        <div style={{ marginTop: 10 }}>
          ...
          <div style={{ display: "flex", gap: 14, marginTop: 6 }}>
            <label><input type="radio" checked={donneesDesag === "1"} onChange={() => setDonneesDesag("1")} /> Oui</label>
            <label><input type="radio" checked={donneesDesag === "2"} onChange={() => setDonneesDesag("2")} /> Non</label>
          </div>
        </div>
        ...
        {donneesDesag === "1" && (
          <div style={{ marginTop: 12, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <select value={desagGenre} onChange={(e) => setDesagGenre(e.target.value as any)}>
              <option value="3">Par Genre ? (Sans objet)</option>
              <option value="1">Par Genre ? Oui</option>
              <option value="2">Par Genre ? Non</option>
            </select>
            <input placeholder="Autre genre (si précisé)" value={desagAutreGenre} onChange={(e) => setDesagAutreGenre(e.target.value)} />

            <select value={desagAge} onChange={(e) => setDesagAge(e.target.value as any)}>
              <option value="3">Age (Sans objet)</option>
              <option value="1">Age Oui</option>
              <option value="2">Age Non</option>
            </select>

            <select value={desagSexe} onChange={(e) => setDesagSexe(e.target.value as any)}>
              <option value="3">Sexe (Sans objet)</option>
              <option value="1">Sexe Oui</option>
              <option value="2">Sexe Non</option>
            </select>

            <select value={desagHandicap} onChange={(e) => setDesagHandicap(e.target.value as any)}>
              <option value="3">Handicap (Sans objet)</option>
              <option value="1">Handicap Oui</option>
              <option value="2">Handicap Non</option>
            </select>

            <select value={desagDecision} onChange={(e) => setDesagDecision(e.target.value as any)}>
            ...
            <select value={desagTerr} onChange={(e) => setDesagTerr(e.target.value as any)}>
            ...
            <select value={desagRegion} onChange={(e) => setDesagRegion(e.target.value as any)}>
            ...
            <select value={desagCercle} onChange={(e) => setDesagCercle(e.target.value as any)}>
            ...
            <select value={desagArr} onChange={(e) => setDesagArr(e.target.value as any)}>
            ...
            <select value={desagCommune} onChange={(e) => setDesagCommune(e.target.value as any)}>
            ...
            <input placeholder="Autre niveau territorial (si précisé)" value={desagAutreTerr} onChange={(e) => setDesagAutreTerr(e.target.value)} />

            <select value={desagMilieu} onChange={(e) => setDesagMilieu(e.target.value as any)}>
            ...
            <select value={desagUrbain} onChange={(e) => setDesagUrbain(e.target.value as any)}>
            ...
            <select value={desagRural} onChange={(e) => setDesagRural(e.target.value as any)}>
            ...
            <select value={desagAutre} onChange={(e) => setDesagAutre(e.target.value as any)}>
            ...
            <input placeholder="Autre niveau à préciser" value={desagAutrePreciser} onChange={(e) => setDesagAutrePreciser(e.target.value)} />
          </div>
        )}

        <div style={{ marginTop: 14 }}>
          <div style={{ fontWeight: 600 }}>2.03 Indicateurs renseignés (multi-choix)</div>
          <div style={{ marginTop: 8, maxHeight: 220, overflow: "auto", border: "1px solid #eee", padding: 10, borderRadius: 8 }}>
            {indicateurs.map((it) => (
        ...
        <div style={{ marginTop: 14, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <div>
            ...
            <select value={envergure} onChange={(e) => setEnvergure(e.target.value)}>
          ...
          <div style={{ gridColumn: "1 / -1" }}>
            ...
            <div style={{ display: "flex", gap: 14, marginTop: 6 }}>
              <label><input type="radio" checked={programmee === "1"} onChange={() => setProgrammee("1")} /> Oui</label>
              <label><input type="radio" checked={programmee === "2"} onChange={() => setProgrammee("2")} /> Non</label>
            </div>
          </div>
        </div>
      </section>
      ...
      <p style={{ marginTop: 18, opacity: 0.7 }}>
        NB: Les données sont stockées dans <b>answers_fiche1.data</b> (JSON). On peut améliorer l’UI plus tard (Tailwind / composants).
      </p>
    </main>
  );
}
